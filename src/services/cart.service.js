...
const getCartByUser = async (user) => {
  const usersCart =  await Cart.findOne({email:user.email})
   if (!usersCart)
...
const addProductToCart = async (user, productId, quantity) => {
  let cart = await Cart.findOne({email:user.email})
    if (!cart){
       try {
        ...
        throw new ApiError(
          httpStatus.INTERNAL_SERVER_ERROR,"User cart creation failed bacause user already have a cart"
        );
       }
    }
  ...
  if (productIndex == -1){
     ...
     cart.cartItems.push({product:product,quantity:quantity});
      ...
      throw new ApiError(
        httpStatus.BAD_REQUEST,"Product already in cart. Use the cart sidebar to update or remove product from cart"
      );
    }
   await cart.save()
   return cart;
...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  let productIdx = -1
   ...
   if (productIdx == -1){
    ...
    cart.cartItems[productIdx].quantity = quantity
   }
    await cart.save()
    return cart
};
